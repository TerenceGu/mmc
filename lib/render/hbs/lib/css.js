  ...
  hbs.registerHelper('cssForPage',
    ({ data: { root: { __selfName, __componentsNames, lang } } }) => {
      if (process.env.COMBO === 'combo') {
        const list = __componentsNames.map(name => `components/${name}`)
          .concat(genName('pages', __selfName));
        return `<link rel="stylesheet" href="${find(list, lang, true)}" crossorigin="anonymous">`;
      }
