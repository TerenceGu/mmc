...
function cssFactory(hbs) {
  ...
  hbs.registerHelper('cssForPage', data => {
    ...
    const componentsPart = pages.map(name => `components/${name}`)
      .map(name => `<link rel="stylesheet" href="${find(name, lang, true)}" crossorigin="anonymous">`)
      .join('');
    const pagePart = `<link rel="stylesheet" href="${find('pages/'+thisPageName, lang, true)}" crossorigin="anonymous">`;
    return `${pagePart}\n${componentsPart}`;
  });
